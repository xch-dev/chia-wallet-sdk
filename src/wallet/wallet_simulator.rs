...
mod tests {
    ...
    fn sign_bundle(spend_bundle: &mut SpendBundle) {
        ...
        for required_signature in required_signatures {
            aggregated_signature += &sign(&sk, &required_signature.final_message());
        }
    }

    #[tokio::test]
    async fn test_coin_lineage_many_blocks() {
        ...
        for _ in 0..1000 {
            let solution = ctx
                .serialize([CreateCoinWithoutMemos {
                    puzzle_hash,
    ...
    #[tokio::test]
    async fn test_spend_unknown_coin() {
        let sim = WalletSimulator::new().await;
        let peer = sim.peer().await;
    ...
    #[tokio::test]
    async fn test_coin_subscriptions() {
        let sim = WalletSimulator::new().await;
        let peer = sim.peer().await;
    ...
    #[tokio::test]
    async fn test_puzzle_subscriptions() {
        let sim = WalletSimulator::new().await;
        let peer = sim.peer().await;
    ...
    #[tokio::test]
    async fn test_hint_subscriptions() {
        let sim = WalletSimulator::new().await;
        let peer = sim.peer().await;
    ...
    #[tokio::test]
    async fn test_request_children() {
        let sim = WalletSimulator::new().await;
        let peer = sim.peer().await;
