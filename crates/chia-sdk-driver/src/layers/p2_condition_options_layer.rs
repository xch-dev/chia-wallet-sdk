...
mod tests {
    ...
    fn test_conditions_options_layer() -> anyhow::Result<()> {
        ...
        let roundtrip = P2ConditionsOptionsLayer::<NodePtr>::parse_puzzle(&ctx, puzzle)?
    ...
    fn test_conditions_options_spend(#[values(0, 1)] option: u16) -> anyhow::Result<()> {
        ...
        let option_coin = Coin::new(alice.coin.coin_id(), p2_puzzle_hash.into(), 1);
        layer.spend(&mut ctx, option_coin, option)?;
        sim.spend_coins(ctx.take(), &[alice.sk])?;

        let solution =
            layer.construct_solution(&mut ctx, P2ConditionsOptionsSolution { option })?;

        let output = run_puzzle(&mut ctx, ptr, solution)?;
