...
impl Clvm {
    ...
    pub fn alloc(&self, value: ClvmType) -> Result<Arc<Program>, ChiaError> {
        let result = match value {
            ...
            ClvmType::Pair { value } => self
                .0
                .pair((*value).0.first.clone(), (*value).0.rest.clone())?,
            ...
            ClvmType::AggSigParent { value } => self
                .0
                .agg_sig_parent((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigPuzzle { value } => self
                .0
                .agg_sig_puzzle((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigAmount { value } => self
                .0
                .agg_sig_amount((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigPuzzleAmount { value } => self
                .0
                .agg_sig_puzzle_amount((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigParentAmount { value } => self
                .0
                .agg_sig_parent_amount((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigParentPuzzle { value } => self
                .0
                .agg_sig_parent_puzzle((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigUnsafe { value } => self
                .0
                .agg_sig_unsafe((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::AggSigMe { value } => self
                .0
                .agg_sig_me((*value).0.public_key.clone(), (*value).0.message.clone())?,
            ClvmType::CreateCoin { value } => self.0.create_coin(
                (*value).0.puzzle_hash,
                (*value).0.amount,
                (*value).0.memos.clone(),
            )?,
            ClvmType::ReserveFee { value } => self.0.reserve_fee((*value).0.amount)?,
            ClvmType::CreateCoinAnnouncement { value } => self
                .0
                .create_coin_announcement((*value).0.message.clone())?,
            ClvmType::CreatePuzzleAnnouncement { value } => self
                .0
                .create_puzzle_announcement((*value).0.message.clone())?,
            ClvmType::AssertCoinAnnouncement { value } => self
                .0
                .assert_coin_announcement((*value).0.announcement_id)?,
            ClvmType::AssertPuzzleAnnouncement { value } => self
                .0
                .assert_puzzle_announcement((*value).0.announcement_id)?,
            ...
            ClvmType::SendMessage { value } => self.0.send_message(
                (*value).0.mode,
            ...
            ClvmType::Softfork { value } => {
                self.0.softfork((*value).0.cost, (*value).0.rest.clone())?
            }
            ...
            ClvmType::UpdateDataStoreMerkleRoot { value } => self.0.update_data_store_merkle_root(
                (*value).0.new_merkle_root,
                (*value).0.memos.clone(),
            )?,
