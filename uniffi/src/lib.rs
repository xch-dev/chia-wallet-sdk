...
impl Clvm {
    ...
    pub fn alloc(&self, value: ClvmType) -> Result<Arc<Program>, ChiaError> {
        let result = match value {
            ...
            ClvmType::Pair { value } => {
                self.0.pair((*value).0.first.clone(), (*value).0.rest.clone())?
            }
            ...
            ClvmType::AggSigParent { value } => {
                self.0.agg_sig_parent((*value).0.public_key.clone(), (*value).0.message.clone())?
            }
            ClvmType::AggSigPuzzle { value } => {
                self.0.agg_sig_puzzle((*value).0.public_key.clone(), (*value).0.message.clone())?
            }
            ClvmType::AggSigAmount { value } => {
                self.0.agg_sig_amount((*value).0.public_key.clone(), (*value).0.message.clone())?
            }
            ...
            ClvmType::Force1of2RestrictedVariableMemo { value } => {
                self.0.force_1_of_2_restricted_variable_memo((*value).0.clone())?
            }
            ...
            ClvmType::NotarizedPayment { value } => {
                self.0.notarized_payment((*value).0.clone())?
            }
